...
.PARAMETER Proxy
    If exist, the script will add the proxy argument to the web request
...
try
{
    $response = Invoke-WebRequest $proxyString -Uri $uri -Method $method -ContentType $contentType -Headers $headers -Body $body -UseBasicParsing
}
catch
{
